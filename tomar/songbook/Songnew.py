...
for cl in cardList:
    ...
    if len(temp[0]) > temp[0].find('') + 1:
        ...
        temp.append(cl[0].split('')[2])   # now the notes are in [6]
    ...
    if str(temp[3]) in deckDict:
        ...
        print("ERROR! {} not found".format(temp[3]))
    temp[4] = cl[4].split() # now tags are a list in [4]
    temp.pop(1) # remove cardId
    temp.pop(1) # remove noteId
    cardDict[cardId] = temp
    cardId += 1
outfile = open('marieSongBook2.json', 'w', encoding='utf8')
json.dump(cardDict, outfile, ensure_ascii=False)
#outfile = open('marieSongBook2.json', 'w')
#json.dump(cardDict, outfile, ensure_ascii=False).encode("utf-8","replace")
outfile.close()
