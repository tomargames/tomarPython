...
#        self.log = open("c:\logs\logfile.log", "a")
#
...
#           #str2 = str.replace('\\u','&#x')
#          
#           str1  = str.encode(encoding = 'utf-8',errors = 'ignore')
#           str1  = str.encode('utf-8','ignore')
#           return str1
...
tag types:
    ...
    P play list
    V voices
    W wikipedia (1 has its own entry, 2 for collection, 3 for artist, 4 no mention)
    Y year
    Z harmonica key
        '''
        ...
        self.tagGroup = {"A":"Artist",
                        ...
                        "Y":"Year",
                        "Z":"Hrmnca"}
        self.tagOrder = ["A", "Y", "G", "H", "C", "K", "B", "M", "P", "V", "W", "O", "Z"]
        self.tagDict = {}
        self.deckDict = {}
...
#        divide into 6 columns, make each a cell
    ...
    def browse(self, sec):
        ...
        return returnHtml
    def displaySongList(self, L, sec):
        ...
        if (sec > '1'):
            returnHtml += '<th style="color: {};">Sheet</th></tr>'.format(columnColors[4])
            noteHtml = '<script>var noteDisplays = {'
        else:
            noteHtml = ''
        for s in L:            #for every song
            ...
            #fix for greek titles while tom looks for a solution
            #print(self.cardDict[s][0])
            self.cardDict[s][0] = GreekWorkaround(self.cardDict[s][0])
            #fix for greek titles while tom looks for a solution
            ...
            if (sec > '1'):
                ...
                if self.cardDict[s][4] != '':
                    #fix for greek titles while tom looks for a solution
                    self.cardDict[s][3] = GreekWorkaround(self.cardDict[s][3])
                    #fix for greek titles while tom looks for a solution
                    returnHtml += '<a class="songDetail" style="text-decoration: none;" href=javascript:showPage("{}")>***</a>'.format(self.cardDict[s][4])
                    noteHtml += '"{}": "{}", '.format(s, self.cardDict[s][4])
                returnHtml += '</td>'
            returnHtml += '</tr>'
        returnHtml += '</table>'
        if (sec > '1'):
            noteHtml += '}; </script>'
        #return (returnHtml, noteHtml)
        #returnHtml1 = html._html5(returnHtml)
        #noteHtml1 = print(noteHtml.decode(sys.getfilesystemencoding())
        # outfile = open('C:/Apache24/htdocs/tomar/songbook/returnHtml.html', 'w', encoding='utf8')
        # outfile.write(returnHtml)
        # outfile.close()
        # outfile = open('C:/Apache24/htdocs/tomar/songbook/noteHtml.html', 'w', encoding='utf8')
        # outfile.write(noteHtml)
        # outfile.close()
